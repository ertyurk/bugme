...
async def add_new_bug(bug_data: dict, authorization: str) -> dict:
    if authorization[0:3] == 'pk_':
        ...
        bug = bug_data
    pre_bug = await bug_collection.insert_one({**bug, 'created_at': str(datetime.now())})
    new_bug = await bug_collection.find_one({
        "_id": pre_bug.inserted_id
    })
...
async def update_bug_data_for_media(id: str, media: list) -> bool:
    bug = await bug_collection.find_one({
        "_id": ObjectId(id)
    })
...
async def retrive_bugs_from_bundle(bundle_id: str, brand_id: str) -> dict:
    bugs = []
    async for bug in bug_collection.find({"bundle_id": bundle_id, "brand_id": brand_id}):
        bugs.append(bugEntity(bug))
    return bugs
