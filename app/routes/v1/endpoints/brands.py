...
async def get_brands():
    ...
    return (
        ResponseModel(brands, "Brands data retrieved successfully")
        if len(brands) > 0
        else ResponseModel(brands, "Empty list returned")
    )
...
async def add_brand(brand: BrandModel = Body(...)):
    ...
    return ResponseModel(
        new_brand, "Brand created successfully.", status.HTTP_201_CREATED
    )


@router.get(
    "/get-api-key/", response_description="Api key for brand generated successfully"
)
...
async def get_brand(id):
    ...
    return (
        ResponseModel(brand, "Brand data retrieved successfully")
        if brand
...
async def update_brand(id: str, brand: UpdateBrandModel = Body(...)):
    ...
    return (
        ...
        if updated_brand
            ...
            "There was an error updating the brand.",
        )
    )
...
async def get_apps_of_a_brand(id):
    ...
    return (
        ResponseModel(apps, "Brand data retrieved successfully")
        if apps
        else ErrorResponseModel("An error occured.", 404, "Brand doesn't exist.")
    )
