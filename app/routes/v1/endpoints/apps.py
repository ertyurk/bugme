...
async def get_apps():
    ...
    return (
        ResponseModel(apps, "apps data retrieved successfully")
        if len(apps) > 0
        else ResponseModel(apps, "Empty list returned")
    )
...
async def get_app(id):
    ...
    return (
        ResponseModel(app, "app data retrieved successfully")
        if app
        else ErrorResponseModel(
            "An error occured.", status.HTTP_404_NOT_FOUND, "App doesn't exist."
        )
    )
...
async def update_app(id: str, app: UpdateAppsModel = Body(...)):
    ...
    return (
        ResponseModel({"id": id}, "App updated successfully")
        if updated_app
        else ErrorResponseModel(
            "An error occurred",
            status.HTTP_404_NOT_FOUND,
            "There was an error updating the App.",
        )
    )


@router.get(
    "/bundle/bugs/", response_description="Bugs retrieved from a brand of an app."
)
async def get_apps_of_brand(bundle_id: str, brand_id: str):
    bugs = await retrive_bugs_from_bundle(bundle_id, brand_id)
    return (
        ResponseModel(bugs, "Brand data retrieved successfully")
        if bugs
        else ErrorResponseModel(
            "An error occured.", status.HTTP_404_NOT_FOUND, "Brand doesn't exist."
        )
    )
...
@router.get(
    "/integrations/slack/",
    response_description="Slack integrations retrieved from brand.",
)
async def retrieve_slack_integrations_of_the_app(app_id: str):
    ...
    return (
        ResponseModel(
            clickups,
            f"Slack integration are retrieved successfully for the App ID: {app_id}",
        )
        if clickups
        else ErrorResponseModel(
...
@router.get(
    ...
    response_description="Clickup integrations retrieved from App.",
)
