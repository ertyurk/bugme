...
async def get_slack_integration_records():
    ...
    return ResponseModel(slacks, "Slack integrations retrieved successfully") \
        if len(slacks) > 0 \
        else ResponseModel(
        slacks, "Empty list returned"
    )
...
async def add_a_slack_integration(slack: SlackModel = Body(...)):
    ...
    return ResponseModel(new_slack, "slack integration created successfully.", status.HTTP_201_CREATED)
...
async def find_a_slack_integration(id):
    ...
    return ResponseModel(slack, "slack data retrieved successfully") \
        if slack \
        else ErrorResponseModel("An error occured.", 404, "slack doesn't exist.")


@router.put('/{id}', response_description="Slack integration data updated in the database.")
async def update_a_slack_integration(id: str, slack: UpdateSlackModel = Body(...)):
    ...
    return ResponseModel({"id": id},
        ...
        if updated_slack \
        else ErrorResponseModel("An error occurred", status.HTTP_404_NOT_FOUND, "There was an error updating the Slack integration.")
...
async def delete_slack_integration(id: str):
    ...
    return ResponseModel("Integration with ID: {} removed".format(id), "Integration deleted successfully") \
        if deleted_slack \
        else ErrorResponseModel("An error occured", 404, "Integration with id {0} doesn't exist".format(id))
