...
from app.integrations.handler import main_background_task_handler, run_integration
...
async def get_bugs():
    ...
    return (
        ResponseModel(bugs, "bugs data retrieved successfully")
        if len(bugs) > 0
        else ResponseModel(bugs, "Empty list returned")
    )


@router.post("/", response_description="Bugs data added into the database.")
async def add_bug(
    bug: BugModel = Body(...), authorization: Optional[str] = Header(None)
):
...
async def get_bug(id):
    ...
    return (
        ResponseModel(bug, "Bug data retrieved successfully")
        if bug
...
async def upload_files(
    bg_task: BackgroundTasks, id: str, files: List[UploadFile] = File(...)
):
    ...
    return ResponseModel(
        "Queue Accepted.",
        "Media file will be updated into relevant bug.",
        status.HTTP_202_ACCEPTED,
    )


@router.get("/{id}/retry/")
async def run_active_integrations(id: str, bg_task: BackgroundTasks):
    bg_task.add_task(run_integration, id)
    return ResponseModel(
        "Queue Accepted.",
        "If the bug that you posted has media and and linked app has any integration, I will run the integration.",
        status.HTTP_202_ACCEPTED,
    )
